...
import type { CommandContext, CommandMatch, ParsedCommand, SlashCommand } from './types.js';
 ...
 */
function fuzzyScore(query: string, name: string): { score: number; highlights: [number, number][] } {
	...
	if (n.startsWith(q)) {
		return { score: 90 + (q.length / n.length) * 10, highlights: [[0, q.length]] };
	}

	// Substring match
	const subIdx = n.indexOf(q);
	if (subIdx !== -1) {
		return { score: 60 + (q.length / n.length) * 20, highlights: [[subIdx, subIdx + q.length]] };
	}
	...
	if (matchStart !== -1) {
		highlights.push([matchStart, matchStart + (qi - highlights.reduce((s, h) => s + h[1] - h[0], 0))]);
	}
...
export function matchCommands(query: string, context: CommandContext): CommandMatch[] {
	...
	for (const command of commands) {
		...
		const score = relevance * 0.6 + fuzzy * 0.4;
 ...
 */
export function getMatchHighlights(name: string, highlights: [number, number][]): HighlightSegment[] {
...
export function buildPromptWithCommand(
	...
	return {
		...
		userMessage: parsed.args || `Help me with ${parsed.command.label.toLowerCase()} for this feature.`,
	};
}
